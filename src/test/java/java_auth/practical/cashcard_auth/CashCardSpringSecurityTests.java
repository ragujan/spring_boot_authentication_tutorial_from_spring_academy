...
public class CashCardSpringSecurityTests {
    ...
    private String mint() {
        return mintToken(consumer -> {
        });
    }

    private String mintToken(Consumer<JwtClaimsSet.Builder> consumer) {
    ...
    void shouldNotAllowTokensThatAreExpired() throws Exception {
        String token = mintToken((claims) -> claims.issuedAt(Instant.now().minusSeconds(3600))
    ...
    void shouldShowAllTokenValidationErrors() throws Exception {
        String expired = mintToken((claims) -> claims
